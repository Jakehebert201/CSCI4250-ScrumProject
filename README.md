...
- When deploying behind nginx under `/app/`, the application automatically respects the prefix via `ProxyFix` + a custom middleware. Local development runs with no prefix by default; set `APP_URL_PREFIX=/app` (or simply forward `X-Script-Name` from nginx) if you need to mimic production paths.
